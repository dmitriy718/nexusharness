...
- **Status:** Confirmation retry required after three diagnosed probe defects. The first attempt exposed premature cleanup (`0x80070003`); the second exposed guest-forced shutdown and a mapped-folder cleanup race (`0x80072746`/`EBUSY`); the third successfully reported `seedRead`, `mappedWrite`, `networkBlocked`, and `sandboxIdentity` as true from `WDAGUtilityAccount`, but Windows PowerShell's UTF-8 BOM caused host `JSON.parse` to fail. Guest output is now written as BOM-free UTF-8 and the host parser also accepts BOM-prefixed results.
