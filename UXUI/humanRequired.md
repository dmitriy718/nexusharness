...
- **Status:** Retry required after two diagnosed probe lifecycle repairs. The first owner attempt on 2026-07-11 reached Windows Sandbox but failed with `0x80070003` because the host launcher returned before the guest initialized, allowing the temporary profile and mapped folder to be cleaned too early. The second attempt reached guest completion but the guest forced its own shutdown, which surfaced as connection loss `0x80072746`, while host cleanup raced the still-mounted cell and raised `EBUSY`. The guest no longer forces shutdown; the host now closes only the newly created Sandbox session and retries transient cleanup locks.
