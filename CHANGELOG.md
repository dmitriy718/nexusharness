...
- Live production trials now have enough bounded Executor turns and no-action continuation recovery for multi-page deliverables, sequential Executors reuse shared work and stay focused on their assigned plan item, every agent role receives the current UTC date, Planners no longer expand research or recommendations into unrequested implementation or copy product requirements from memory, concise Planner output receives a larger non-truncating budget, new local runtimes allow five silent minutes for large tool payloads, failed read-only validation returns exact diagnostics without terminalizing the transaction, and Glassbox throttles cumulative stream upserts instead of sending quadratic token-by-token payloads.
- Closed a severity-one self-host containment path: execution now fails closed unless a mode is explicitly configured, compatibility host execution requires a separate acknowledgement and Approval mode, and realpath checks reject workspaces that overlap the NexusHarness installation. Concurrent runs retain independent task exports and transaction cells while duplicate activation of the same run is rejected.
