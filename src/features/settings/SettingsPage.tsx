...
function WorkspaceSettings({ draft, setDraft, errors }: { draft: SettingsShape; setDraft: (value: SettingsShape) => void; errors: Record<string, string> }) {
  return <><div className="setting-callout"><FolderCode /><div><strong>Workspace boundary</strong><p>Every built-in file and shell capability resolves against this existing directory. Saving revalidates it on the server.</p></div></div><FieldError error={errors.workspaceRoot}><Field label="Workspace root" htmlFor="settings-workspace" hint="Existing absolute directory; paths cannot escape it."><input id="settings-workspace" aria-invalid={Boolean(errors.workspaceRoot)} value={draft.workspaceRoot} onChange={(event) => setDraft({ ...draft, workspaceRoot: event.target.value })} /></Field></FieldError><div className="form-grid"><Field label="Test command" htmlFor="settings-test" hint="Optional objective-validation command."><input id="settings-test" value={draft.testCommand} onChange={(event) => setDraft({ ...draft, testCommand: event.target.value })} placeholder="npm test" /></Field><Field label="Lint command" htmlFor="settings-lint" hint="Optional objective-validation command."><input id="settings-lint" value={draft.lintCommand} onChange={(event) => setDraft({ ...draft, lintCommand: event.target.value })} placeholder="npm run lint" /></Field></div><FieldError error={errors.shellPath}><Field label="Shell executable" htmlFor="settings-shell" hint="Executable used for approved validation and agent shell commands."><input id="settings-shell" aria-invalid={Boolean(errors.shellPath)} value={draft.shellPath} onChange={(event) => setDraft({ ...draft, shellPath: event.target.value })} /></Field></FieldError></>;
}
...
function NumberSetting({ id, label, value, min, max, unit, error, onChange }: { id: string; label: string; value: number; min: number; max: number; unit: string; error?: string; onChange: (value: string) => void }) {
  return <FieldError error={error}><Field label={label} htmlFor={id} hint={`${min.toLocaleString()}–${max.toLocaleString()} ${unit}`}><div className="number-control"><input id={id} aria-invalid={Boolean(error)} type="number" min={min} max={max} value={value} onChange={(event) => onChange(event.target.value)} /><span>{unit}</span></div></Field></FieldError>;
}

function FieldError({ error, children }: { error?: string; children: React.ReactNode }) {
  return <div className="field-with-error">{children}{error && <p className="field-error" role="alert">{error}</p>}</div>;
}
