...
function RuntimeWizard({ draft, updateDraft, errors, tested, testing, saving, formError, testConnection, saveRuntime, close }: { draft: RuntimeDraft; updateDraft: <K extends keyof RuntimeDraft>(key: K, value: RuntimeDraft[K]) => void; errors: ReturnType<typeof validateRuntimeDraft>; tested: RuntimeTestResult | null; testing: boolean; saving: boolean; formError: string; testConnection: () => Promise<void>; saveRuntime: () => Promise<void>; close: () => void }) {
  ...
  return <section className="section-block runtime-wizard" aria-labelledby="runtime-wizard-title">
    ...
    <div className="runtime-wizard-grid"><div className="runtime-form">
      ...
      {httpRuntime ? <><Field label="Endpoint URL" htmlFor="runtime-endpoint" hint="The local HTTP endpoint exposed by the runtime."><input id="runtime-endpoint" aria-invalid={Boolean(errors.endpoint)} aria-describedby={errors.endpoint ? "runtime-endpoint-hint runtime-endpoint-error" : "runtime-endpoint-hint"} value={draft.endpoint} onChange={(event) => updateDraft("endpoint", event.target.value)} /></Field>{errors.endpoint && <small className="field-error" id="runtime-endpoint-error" role="alert">{errors.endpoint}</small>}</> : <><Field label="llama.cpp executable" htmlFor="runtime-binary" hint="Absolute path to llama-cli or llama-cli.exe."><input id="runtime-binary" aria-invalid={Boolean(errors.binaryPath)} aria-describedby={`runtime-binary-hint${errors.binaryPath ? " runtime-binary-error" : ""}`} value={draft.binaryPath} onChange={(event) => updateDraft("binaryPath", event.target.value)} placeholder="C:\\llama.cpp\\llama-cli.exe" /></Field>{errors.binaryPath && <small className="field-error" id="runtime-binary-error" role="alert">{errors.binaryPath}</small>}<Field label="GGUF model file" htmlFor="runtime-model-path"><input id="runtime-model-path" aria-invalid={Boolean(errors.modelPath)} aria-describedby={errors.modelPath ? "runtime-model-path-error" : undefined} value={draft.modelPath} onChange={(event) => updateDraft("modelPath", event.target.value)} placeholder="D:\\models\\model.gguf" /></Field>{errors.modelPath && <small className="field-error" id="runtime-model-path-error" role="alert">{errors.modelPath}</small>}</>}
      <Field label="Response inactivity timeout" htmlFor="runtime-timeout" hint="Milliseconds without response activity; streamed Ollama output resets this timer. Range: 1,000–300,000."><input id="runtime-timeout" aria-invalid={Boolean(errors.timeoutMs)} aria-describedby={`runtime-timeout-hint${errors.timeoutMs ? " runtime-timeout-error" : ""}`} type="number" min="1000" max="300000" value={draft.timeoutMs} onChange={(event) => updateDraft("timeoutMs", Number(event.target.value))} /></Field>{errors.timeoutMs && <small className="field-error" id="runtime-timeout-error" role="alert">{errors.timeoutMs}</small>}
