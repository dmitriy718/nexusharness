...
function localToolSchemas() {
  return [
    ...
    { type: "function", function: { name: "file_delete", description: "Delete a file or directory inside the configured workspace root. Requires approval when enabled.", parameters: { type: "object", required: ["path"], properties: { path: { type: "string" } } } } },
    { type: "function", function: { name: "shell_exec", description: "Run a shell command in the configured workspace root. Requires approval when enabled.", parameters: { type: "object", required: ["command"], properties: { command: { type: "string" } } } } }
  ];
}
...
async function runModelTurn(role: AgentRole, messages: ChatMessage[], bindings: RoleBindings, signal: AbortSignal) {
  const { runtime, model } = bindings[role];
  const tools = role === "executor" ? await availableTools() : undefined;
...
async function runExecutorSubtask(runId: string, task: string, plan: string[], subtask: string, criticText: string, bindings: RoleBindings, signal: AbortSignal, previousOutput = ""): Promise<string> {
  ...
  for (let toolRound = 0; toolRound < 16; toolRound += 1) {
    ...
    const response = await runModelTurn("executor", executorMessages, bindings, signal);
...
async function runValidation(store: StoreShape, run: TaskRun, signal: AbortSignal): Promise<string> {
  const commands = [
...
export async function executeRun(runId: string) {
  ...
  try {
    ...
    for (let iteration = firstIteration; iteration <= run.maxIterations; iteration += 1) {
      ...
      if (!skipExecutionOnce) {
        ...
        const subtaskResults = await runExecutorBatch(
          ...
          revisionPlan,
          criticText,
          iteration === 1 ? store.settings.maxParallelExecutors : 1,
          bindings,
          controller.signal,
          executorOutput
        );
      ...
      await saveStore(store);
      try {
        validationOutput = await runValidation(store, run, controller.signal);
    ...
    if (failed) {
      ...
      failed.status = canceled ? "canceled" : String(error.message ?? error).includes("Approval required") ? "waiting_approval" : "failed";
      failed.error = canceled ? "Run canceled by operator." : error.message ?? String(error);
      failed.updatedAt = new Date().toISOString();
      await saveStore(latest);
    }
    if (!controller.signal.aborted) {
      await audit({ actor: "system", action: "run.error", risk: "execute", status: "error", message: error.message ?? String(error) });
    }
  } finally {
    activeRuns.delete(runId);
  }
